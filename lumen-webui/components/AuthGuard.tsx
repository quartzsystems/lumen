...
import { ApiError, fetchMe, getCurrentUser } from "@/lib/authClient";

/// Auth gate for console pages. The session is an httpOnly cookie (invisible
...
/// failure means the control plane is down or restarting — bouncing to /login
/// then would ping-pong with its "already signed in" redirect, so we keep the
/// console up (pages surface their own errors) or show a retry screen when
    ...
    return (
      ...
      >
        <p className="text-[13px] m-0" style={{ color: "var(--qz-danger)" }}>
          Cannot reach the Lumen control plane. It may be restarting.
        </p>
        <button
          onClick={() => {
            setState("checking");
